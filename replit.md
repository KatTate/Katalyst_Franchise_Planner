...
- **Quick Entry Mode:** Spreadsheet-style grid built on `@tanstack/react-table` with `@tanstack/react-virtual` row virtualization (`client/src/components/planning/quick-entry-mode.tsx`, `client/src/components/planning/editable-cell.tsx`). Features collapsible category groups, inline cell editing, sticky summary metrics, per-field source badges, brand default display, reset-to-default, Gurple (#A9A2AA) out-of-range advisory indicators with tooltips, auto-formatting (currency $X,XXX.XX / percentage X.X% / integer on blur, raw values on focus), and keyboard navigation (Tab/Shift+Tab/Enter/Escape). Reuses field metadata from `client/src/lib/field-metadata.ts`.
