...
1. Given I am on the What-If Playground (the "scenarios" workspace view) with sensitivity sliders visible, when the chart dashboard renders below the sliders panel, then 6 chart cards are displayed in a responsive 2-column grid (2×3 on desktop ≥1024px, 1-column stack on mobile <768px). The chart area is labeled "Your Business — Base vs Your Scenario" or similar contextual heading.

2. Given the What-If Playground has loaded and `SensitivityOutputs` (base, current) are computed, when any slider value changes and new `SensitivityOutputs` are recomputed (debounced by Story 10.1), then all 6 chart containers re-render with updated data within the same React commit cycle — no chart displays stale data from a previous computation.

### Chart 1 — "Am I Making Money?" (Profitability)

3. Given `SensitivityOutputs` are available, when Chart 1 renders, then it shows a 5-year line/area chart with two scenario curves (Base Case and Your Scenario) for all five P&L metrics from `annualSummaries`: **Annual Revenue** (`revenue`, primary area), **COGS** (`totalCogs`), **Gross Profit** (`grossProfit`), **EBITDA** (`ebitda`), and **Pre-Tax Income** (`preTaxIncome`) — all rendered as overlaid lines. The chart X-axis shows Year 1 through Year 5; Y-axis shows dollar amounts. Base Case is rendered as a solid line, Your Scenario as a dashed line (`strokeDasharray="5 5"`). If a saved scenario is loaded for comparison (Story 10.3), a third dotted curve is shown.
   - Source: `_bmad-output/planning-artifacts/epics.md` Story 10.2a; `ux-design-specification-consolidated.md` Journey 4
...
5. Given `SensitivityOutputs` are available, when Chart 2 renders, then it shows a 60-month line chart (monthly granularity from `monthlyProjections`) with two scenario curves (Base Case and Your Scenario) for **Ending Cash Balance** (`endingCash`). The X-axis shows months 1–60 (with year markers); the Y-axis shows dollar amounts. Monthly granularity is required so amber advisory zones can identify specific months where cash dips negative, not just year-end snapshots.
...
7. Given `SensitivityOutputs` are available, when Chart 3 renders, then it shows the months-to-break-even for each scenario as a horizontal bar chart (or annotated timeline visualization). Each scenario bar shows:
   - **Base Case**: `roiMetrics.breakEvenMonth` from `SensitivityOutputs.base`
   - **Your Scenario**: `roiMetrics.breakEvenMonth` from `SensitivityOutputs.current`

   If a saved scenario is loaded for comparison (Story 10.3), a third bar shows its break-even month.
...
8. Given Chart 3 renders, when I view the chart, then each bar is labeled with its scenario name and the break-even month value (e.g., "Base: Month 14", "Your Scenario: Month 22"). The bars use appropriate colors: primary for Base Case, a distinct color for Your Scenario.

### Chart 4 — "What's My Return?" (ROI & Returns)

9. Given `SensitivityOutputs` are available, when Chart 4 renders, then it shows a 5-year cumulative ROIC line chart. The Y-axis shows ROIC percentage; the X-axis shows Year 1 through Year 5. Two scenario curves display `roicExtended[].roicPct` for years 1-5 (Base solid, Your Scenario dashed).

10. Given Chart 4 renders, then a plain-language callout card appears below or beside the chart summarizing the user's scenario. The specific phrasing is: "Your scenario: [X]% ROIC at Year 5." Do NOT say "Even in the conservative scenario..." or reference system-defined scenarios.

### Chart 5 — "Is My Business Growing?" (Balance Sheet Health)

11. Given `SensitivityOutputs` are available, when Chart 5 renders, then it shows a 5-year dual-line chart for **Total Assets** (`annualSummaries[].totalAssets`) vs **Total Liabilities** (`annualSummaries[].totalLiabilities`) for both scenarios (Base Case and Your Scenario). The growing gap between assets and liabilities lines represents equity growth. The X-axis shows Year 1 through Year 5; the Y-axis shows dollar amounts.

### Chart 6 — "How's My Debt?" (Debt & Working Capital)

12. Given `SensitivityOutputs` are available, when Chart 6 renders, then it shows a 5-year chart with two data series:
    ...
    Two scenario curves are displayed for each series (Base Case solid, Your Scenario dashed). The X-axis shows Year 1 through Year 5.

### Error Boundary

13. Given `SensitivityOutputs` computation throws an error or returns malformed data (e.g., `annualSummaries` has fewer than 5 entries, `roicExtended` is empty, or any required field is undefined), when the chart dashboard attempts to render, then an error boundary catches the failure and displays a user-friendly message (e.g., "Unable to generate charts. Please try adjusting your inputs.") without crashing the entire What-If Playground.

### Performance

14. Given all 6 charts are rendered with two scenarios and a slider value changes, when recomputation completes, then the full chart update completes within 500ms on a standard device (verified via Chrome DevTools Performance audit at 4x CPU throttle). Use `React.memo` on `SensitivityCharts` and `useMemo` per chart. If Chart 2 (60-month, 120 data points for 2 scenarios) causes jank, consider downsampling to monthly averages per quarter for display while keeping full data in tooltips.
...
16. Given the charts are rendered, then scenario curves use consistent colors: primary color for Base Case (solid line), a distinct secondary color for Your Scenario (dashed line). If a comparison scenario is loaded (Story 10.3), it uses a third color with dotted line style. Advisory zones (cash-negative amber on Chart 2) use amber advisory color (never error-red), consistent with the Guardian Bar visual language (FR88–FR89). Use CSS chart color tokens (`--chart-1` through `--chart-5`) for distinct data series within each chart.
...
- **Epic 10 is the sole scenario analysis surface (SCP-2026-02-20 D5/D6):** The column-splitting scenario comparison was retired from Reports. Epic 10 (What-If Playground) is now the **canonical and only** home for scenario analysis in the product. `ScenarioBar`, `ComparisonTableHead`, and `ScenarioSummaryCard` remain in the codebase as dead code pending cleanup — do not integrate them.
...
- **Scenario computation (revised per SCP-2026-02-21):** `computeSensitivityOutputs()` in `client/src/lib/sensitivity-engine.ts` accepts `(planInputs, startupCosts, currentSliders)` and returns `SensitivityOutputs { base, current }` — each being a full `EngineOutput`. The Conservative/Optimistic scenarios have been removed (SCP-2026-02-21 D1). Story 10.2a receives `SensitivityOutputs` as a prop.
  - Source: `client/src/lib/sensitivity-engine.ts` → `computeSensitivityOutputs()`, `SensitivityOutputs`
...
- **Scenario line styles:**
  - Base Case (solid line): primary color
  - Your Scenario (dashed line, `strokeDasharray="5 5"`): distinct secondary color
  - Comparison scenario (dotted line, `strokeDasharray="2 2"`, Story 10.3): third distinct color
...
- **Data memoization:** Chart data transforms (mapping `annualSummaries` or `monthlyProjections` to chart data arrays) MUST be wrapped in `useMemo`. Each of the 6 charts should memoize its data transform independently, keyed on the `SensitivityOutputs` prop. This prevents unnecessary recalculations on unrelated re-renders.
...
- **Plan data access:** The `WhatIfPlayground` component (created in 10.1) receives `planId`. It computes `SensitivityOutputs` (from `computeSensitivityOutputs`) and passes them to `SensitivityCharts` as a prop. Story 10.2a's `SensitivityCharts` component receives the computed `SensitivityOutputs` as a prop — it does NOT access plan data directly.
...
- Heading: e.g., "Business Impact — Base vs Your Scenario" (contextual label, not prescriptive)
...
- Chart legend: inline or below chart — scenario labels: "Base Case" / "Your Scenario" with color dots matching line colors
...
**Loading state:**
- If `SensitivityOutputs` is `null` or still computing, chart cards show skeleton placeholders at the same height as the charts (`Skeleton className="h-[220px] w-full"`)
...
- **DO NOT import or use `ScenarioBar` or `ScenarioSummaryCard` from Epic 5 statements.** Those components are officially marked `[DEAD CODE]` in `architecture.md` per SCP-2026-02-20 D5/D6.

- **DO NOT hardcode arbitrary hex or HSL color values for scenario lines.** Use CSS chart tokens and appropriate color constants. This ensures dark mode works correctly.

- **DO NOT add new API endpoints or server-side calls.** All scenario computation is client-side (`calculateProjections` imported from `shared/financial-engine.ts`). Chart data is derived from `SensitivityOutputs` in-memory. No server fetching for charts.
...
- **Story 10.1 passes the UNMODIFIED saved plan inputs to `computeSensitivityOutputs`.** The `Base` scenario always reflects the user's saved plan as-is. Slider adjustments are applied internally to produce `current`. Story 10.2a only consumes the resulting `SensitivityOutputs` prop — it does not touch slider state or plan inputs.
...
- **`ChartContainer` requires both a `config: ChartConfig` prop and a `className`.** Always pass `className="h-[220px] w-full"` or similar. Without a height class, the chart will collapse to 0px.
