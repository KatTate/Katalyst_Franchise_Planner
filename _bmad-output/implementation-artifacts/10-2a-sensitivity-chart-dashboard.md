...
3. Given `SensitivityOutputs` are available, when Chart 1 renders, then it shows a 5-year line/area chart for all five P&L metrics from `annualSummaries`: **Annual Revenue** (`revenue`, primary area), **COGS** (`totalCogs`), **Gross Profit** (`grossProfit`), **EBITDA** (`ebitda`), and **Pre-Tax Income** (`preTaxIncome`) — all rendered as overlaid lines. The chart X-axis shows Year 1 through Year 5; Y-axis shows dollar amounts. Two scenario curves are shown: Base Case as a solid line, Your Scenario as a dashed line (`strokeDasharray="5 5"`). If a saved scenario is loaded for comparison (Story 10.3), a third curve (dotted line, `strokeDasharray="3 3"`) shows the comparison scenario. See "Scenario line colors" in Dev Notes for current resolved values.
   - Source: `_bmad-output/planning-artifacts/epics.md` Epic 10; SCP-2026-02-21 D1/D2
...
5. Given `SensitivityOutputs` are available, when Chart 2 renders, then it shows a 60-month line chart (monthly granularity from `monthlyProjections`) with two scenario curves (Base Case solid, Your Scenario dashed) for **Ending Cash Balance** (`endingCash`). If a comparison scenario is loaded (Story 10.3), a third dotted curve is shown. The X-axis shows months 1–60 (with year markers); the Y-axis shows dollar amounts. Monthly granularity is required so amber advisory zones can identify specific months where cash dips negative, not just year-end snapshots.
...
7. Given `SensitivityOutputs` are available, when Chart 3 renders, then it shows the months-to-break-even as a horizontal bar chart (or annotated timeline visualization). Bars shown:
   ...
   - **Comparison** (optional, if a saved scenario is loaded via Story 10.3): `roiMetrics.breakEvenMonth` from the comparison output
...
8. Given Chart 3 renders, when I view the chart, then each bar is labeled with its scenario name and the break-even month value (e.g., "Base: Month 14", "Your Scenario: Month 22"). The bars use distinct colors for Base Case and Your Scenario.

### Chart 4 — "What's My Return?" (ROI & Returns)

9. Given `SensitivityOutputs` are available, when Chart 4 renders, then it shows a 5-year cumulative ROIC line chart. The Y-axis shows ROIC percentage; the X-axis shows Year 1 through Year 5. Two scenario curves display `roicExtended[].roicPct` for years 1-5: Base Case (solid), Your Scenario (dashed). If a comparison scenario is loaded, a third curve (dotted) is shown.

10. Given Chart 4 renders, then a plain-language callout card appears below or beside the chart summarizing Your Scenario's result. The specific phrasing is: "Your scenario: [X]% ROIC at Year 5." If sliders are all at zero, show "Base case: [X]% ROIC at Year 5." Do NOT say "Even in the conservative scenario..." (per UX spec advisory language constraint).

### Chart 5 — "Is My Business Growing?" (Balance Sheet Health)

11. Given `SensitivityOutputs` are available, when Chart 5 renders, then it shows a 5-year dual-line chart for **Total Assets** (`annualSummaries[].totalAssets`) vs **Total Liabilities** (`annualSummaries[].totalLiabilities`) for Base Case and Your Scenario. The growing gap between assets and liabilities lines represents equity growth. Each scenario's asset line uses distinct styling (Base solid, Your Scenario dashed), with liabilities as a lighter/muted variant. The X-axis shows Year 1 through Year 5; the Y-axis shows dollar amounts. If a comparison scenario is loaded, a third set of dotted lines is shown.
...
12. Given `ScenarioOutputs` are available, when Chart 6 renders, then it shows a 5-year chart with two data series:
    ...
    Two scenario curves (Base Case solid, Your Scenario dashed) are displayed for each series. If a comparison scenario is loaded (Story 10.3), a third set of dotted curves is shown. The X-axis shows Year 1 through Year 5.
...
16. Given the charts are rendered, then scenario curves use consistent, distinct colors: Base Case (solid line), Your Scenario (dashed line), and optional comparison scenario (dotted line). Do NOT hardcode arbitrary HSL values in components — use runtime constants or CSS tokens. Advisory zones (cash-negative amber on Chart 2) use amber advisory color (never error-red), consistent with the Guardian Bar visual language (FR88–FR89). Use CSS chart color tokens (`--chart-1` through `--chart-5`) for distinct data series within each chart.
...
- **Scenario computation (ALREADY EXISTS — DO NOT REIMPLEMENT):** `computeSensitivityOutputs()` in `client/src/lib/sensitivity-engine.ts` accepts `(planInputs, startupCosts, currentSliders)` and returns `SensitivityOutputs { base, current }` — each being a full `EngineOutput`. Story 10.1 calls this function and passes the results down. Story 10.2a receives `SensitivityOutputs` as a prop. Per SCP-2026-02-21 D1/D5, the old Conservative/Optimistic system columns have been removed — only Base and Your Scenario (current slider state) are computed.
...
- **Scenario line colors:**
  - Base Case (solid line): `hsl(var(--primary))` (Katalyst Green) or equivalent runtime constant
  - Your Scenario (dashed line, `strokeDasharray="5 5"`): a distinct color — e.g., `hsl(var(--chart-2))` or orange-family
  - Comparison scenario (dotted line, `strokeDasharray="3 3"`, only when loaded via Story 10.3): a third distinct color — e.g., blue-family
...
- Heading: e.g., "Your Business — Base vs Your Scenario" (contextual label, not prescriptive)
...
- Chart legend: inline or below chart — scenario labels: "Base Case" / "Your Scenario" (+ "Comparison" when loaded) with color dots matching line colors
...
- **Story 10.1 passes the UNMODIFIED saved plan inputs to `computeSensitivityOutputs` — do NOT apply slider state to `PlanFinancialInputs` before calling it.** The `Base` scenario always reflects the user's saved plan as-is. Slider adjustments are applied internally within `computeSensitivityOutputs` to produce the `current` output. Story 10.2a only consumes the resulting `SensitivityOutputs` prop — it does not touch slider state or plan inputs.
  - Source: `_bmad-output/planning-artifacts/epics.md` Story 10.1 ("slider adjustments do NOT modify the user's actual plan", "Base case always reflects the user's actual saved plan inputs — not slider-modified values")
...
- **Conservative/Optimistic system columns have been removed (SCP-2026-02-21 D1).** The old three-scenario model (Base/Conservative/Optimistic) is retired. Charts now show Base Case + Your Scenario (current slider state). Story 10.2a charts always render from the current `SensitivityOutputs` prop — the computation source is Story 10.1's responsibility.
  - Source: SCP-2026-02-21 D1, D2, D5
...
- **Chart 2 performance note:** 2 scenarios × 60 data points = 120 line segments (3 if comparison loaded). If animation causes jank on lower-end devices, consider downsampling to monthly averages per quarter for the line display while retaining full monthly data in tooltips.
